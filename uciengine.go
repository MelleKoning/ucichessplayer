// Package ucichessplayer implements the UCI protocol for communicating with
// chess engines. It's depended on the chess and board representation of the github.com/notnil/chess package
// and a port of the uciengine from github.com/malbrecht/chess
// ported to be based on github.com/notnil/chess board representation.
...
// UciEngine provides a generic interface to a running chess engine.
...
// ReadablePv is for uci output of the returned principal variation
func (p *Pv) ReadablePv(position *chess.Position) string {
	var pvstring string
	pvPosition := position
	lan := chess.LongAlgebraicNotation{}
	for _, m := range p.Moves {
		pvstring += lan.Encode(pvPosition, &m) + " "
		pvPosition.Update(&m) // execute move
	}
	return pvstring
}
